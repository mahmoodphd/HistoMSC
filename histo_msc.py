...
# def _extract_partition_contours(p, apron = 10):
...
#         for pp in cc[0]:
#             pp[0] = float(pp[0] - delta) / float(p.shape[0])
#             pp[1] = float(pp[1] - delta) / float(p.shape[1])
...
#     return msc_cont, contours

# def polygon_from_box(b):
#     pol =  [ [ b[0]-b[2]/2 ,b[1]-b[3]/2 ] , [ b[0]+b[2]/2, b[1]-b[3]/2 ], [ b[0]+b[2]/2 , b[1]+b[3]/2 ], [ b[0]-b[2]/2 , b[1]+b[3]/2 ], [ b[0]-b[2]/2 ,b[1]-b[3]/2 ] ] 
#     gp = geojson.Polygon( [pol] )
#     return gp

# def polygon_glyph(p, color, name, color_rgb, size = 400.0):
#     pol = [ [ p[0]- size/2 , p[1]] , [ p[0], p[1]-size/2], 
#            [ p[0]+size/2, p[1]], [p[0], p[1]+size/2], [p[0]-size/2,p[1]]]
#     gp = geojson.Polygon( [pol] )
#     properties = {}
...
#     colors = [cm.get_cmap(cmap)(1.0)[:3] for cmap in config['kde_colormap']]
    
#     for class_id in config['class_id']:
#         msc_cont, contours = kde_contours_fft(x, y, labels, p_all, class_id, roi,
#                                             threshold=config['threshold'],
#                                             filtering=config['kde_filtering'],
#                                             weighting=config['kde_weighting'],
#                                             cmap=config['kde_colormap'][class_id],
#                                             debug_pic=f'test_kde_{class_id}.png',
#                                             kernel=config['kde_kernel'],
#                                             norm=config['kde_norm'],
#                                             bw=config['kde_bandwidth'],
#                                             gamma=config['gamma'],
#                                             contrast=config['contrast'])
        
#         if msc_cont == 0:  # Skip if no contours found
#             continue
            
#         # Process MSC contours
#         for contour in msc_cont:
#             feature = {
#                 "type": "Feature",
...
#                 },
#                 "properties": {
#                     "color": [int(255*c) for c in colors[class_id]],
#                     "classification": {
#                         "name": config['classnames'][class_id],
#                         "colorRGB": colorRGB(colors[class_id])
#                     },
#                     "isLocked": False,
#                     "measurements": []
#                 }
...
#                 "geometry": {
#                     "type": "Polygon",
#                     "coordinates": [[[float(p[0]), float(p[1])] for p in contour]]
#                 },
#                 "properties": {
#                     "color": [int(255*c) for c in colors[class_id]],
#                     "classification": {
...
#     # Save features as GeoJSON if output path is available
#     if 'output_dir' in config and 'slide_id' in config:
#         output_path = os.path.join(config['output_dir'], f"{config['slide_id']}_sig.json")
#         with open(output_path, 'w') as f:
#             json.dump(features, f, indent=2)
    
#     return features, [], []  # Return empty lists for backward compatibility
...
# (methodName, method) = OPENCV_METHODS
...
#         i,xv,yv,wv,hv,l,p = squeezenet_inference(model_dir,imgdir)
...
#     annotations, partitions, critical = kde_contours_all(xv,yv,l,p,config)
